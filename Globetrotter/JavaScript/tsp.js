...
async function travellingsalesman() {
    ...
    if (shortest_path[shortest_path.length - 1] == Infinity) {
        ...
        //Checking sufficient energy is there or not
        if ((new_energy - (0.5 * (shortest_path[shortest_path.length - 1] - destination.length))) >= 0) {
            ...
            if (!abort) {
                ...
                new_energy -= 0.5 * shortest_path[shortest_path.length - 1];
                display_energy();
            }
        } else {
            energy_low();
